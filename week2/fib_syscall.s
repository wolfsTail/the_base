...
.globl _start  # метка точки входа
    fib:
        ...
        testq %rdi, %rdi
        jz ok_fib
        movq %rdi, %rcx  # перенесли в counter n
        movq $1, %rdi
    while_fib:
        movq %rdi, %rsi
        addq %rax, %rdi
    ...
    print_num:  # скопировал с add_syscall
        ...
        while_print:  # метка цикла while
            ...
            jnz while_print  # проверяем ьиты в rflags и в случае если они не установлены идем по тметке
    ...
    _start:
        movq $10, %rdi  # n - 10ое число Фибоначи
        call fib # в rax результат
